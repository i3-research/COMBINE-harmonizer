...
load_data_dict <- function(filename, sheet_name) {
    ...
    is_valid_var <- !is.na(df_data_dict[[COMBINEHarmonizer::VAR_NAME]])
    is_valid_type <- !is.na(df_data_dict[[COMBINEHarmonizer::VAR_TYPE]])
    ...
    sheet_names <- lapply(
        df_data_dict_valid[[COMBINEHarmonizer::CATEGORY]],
        \(x) COMBINEHarmonizer::CATEGORY_PERIOD_MAP[[x]]
    )
...
data_dict_fill_category <- function(df) {
    tidyr::fill(df, COMBINEHarmonizer::CATEGORY)
}
